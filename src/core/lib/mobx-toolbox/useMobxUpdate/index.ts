...
import { saiLocalCacheUpdater, saiLocalStorageUpdater } from '../mobxSaiWs';
...
export class MobxUpdater {
	...
	/** 
	 * Returns the update function for array or object
	 * 
	...
	/** 
	 * Updates state of an element in array or object.
	 * 
	 * @param arrayOrObject - Массив или объект, который нужно обновить.
	...
	/** 
 * Helper for deep update by path. Missing path or key will be created.
 * 
...
/**
 * Helper to update state of an array or object (works with MobX arrays).
 * Telegram: https://t.me/nics51
 *
 * @example
 * const commentsUpdate = useMobxUpdate(commentsList);
 * onClick={() => commentsUpdate(comment.id, "count", (prev) => prev + 1)}
 *
 * @param arrayOrObjectGetter - Array or object to update
 * @param annotations - MobX annotations object, e.g. { name: observable }
 * @returns Function to update an element's state
 */
