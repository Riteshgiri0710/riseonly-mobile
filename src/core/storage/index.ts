	...
	async get<T>(key: string): Promise<T | null> {
		try {
			...
			try {
				...
				// Non-JSON values (e.g. plain strings like file paths) cause parse to fail
				if (__DEV__) {
					console.warn(`[Storage] Invalid JSON for key "${key}", returning null. First char: "${value.charAt(0)}"`);
				}
				return null;
			}
		} catch (error) {
