...
export const BottomSheetUi = observer(({
   ...
   const displayTitle = navTitle ?? (title ? String(title) : '');
   ...
   const handleSheetClose = useCallback(() => {
      ...
      if (setIsBottomSheet) {
         BottomSheetState.isProgrammaticClose = false;
         BottomSheetState.isOpen = false;
         BottomSheetState.isFullyOpened = false;
         BottomSheetState.isAnimating = false;

         setIsBottomSheet(false);

         if (setOnCloseSignal) {
            setOnCloseSignal(false);
         }
      }
   }, [setIsBottomSheet, setOnCloseSignal]);

   const handleOnCloseSignal = useCallback(() => {
      if (!bottomSheetRef.current) {
         if (setOnCloseSignal) setOnCloseSignal(false);
         return;
      }

      BottomSheetState.isProgrammaticClose = true;
   ...
   }, [footerAnimation, setOnCloseSignal]);
   ...
   useEffect(() => {
      console.log('[BottomSheetUi] useEffect onCloseSignal:', onCloseSignal);

      if (!onCloseSignal) return;

      console.log('[BottomSheetUi] Calling handleOnCloseSignal from useEffect');
      handleOnCloseSignal();
   }, [onCloseSignal]);
   ...
   const handleSheetChanges = useCallback((index: number) => {
      ...
      if (index === -1) {
         ...
         BottomSheetState.isFullyOpened = true;
   ...
   useEffect(() => {
      const keyboardWillShow = Keyboard.addListener(
         ...
         (e) => {
            ...
            if (bottomSheetRef.current && isBottomSheet && snapPoints) {
               setProgrammaticSnapChange(true);
                  ...
                  >
                     {displayTitle && (
                        ...
                        >
                           ...
                           {/* Animated title on swipe */}
                           {swipeTitleState ? (
                              <View style={styles.titleTextContainer}>
                                 {/* Current title (exiting) */}
                                 <MainText
                                    px={16}
                                    tac='center'
                                    width={"100%"}
