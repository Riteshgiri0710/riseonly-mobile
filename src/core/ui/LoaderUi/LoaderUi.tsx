	...
	if (type === "progress") {
		const content = (
			<View
				style={[
					...
					styles.closeIconContainer,
					{
						...
						height: circleSize,
						borderRadius: 1000,
						backgroundColor: debug ? "red" : "rgba(0, 0, 0, 0.8)",
						overflow: "hidden",
			...
			>
				...
				{closeCallback && (
					<View style={[styles.closeIconContainer, { width: "100%", height: "100%" }]}>
						<MaterialCommunityIcons name="close" size={getCloseIconSize()} color="#fff" />
					</View>
				)}
			</View>
		);
		if (closeCallback) {
			return (
				<Pressable onPress={closeCallback} style={[styles.container, { width: circleSize, height: circleSize }, debug && { backgroundColor: "red" }]}>
					{content}
				</Pressable>
			);
		}
		return <View style={[styles.container, { width: circleSize, height: circleSize }]}>{content}</View>;
	}
