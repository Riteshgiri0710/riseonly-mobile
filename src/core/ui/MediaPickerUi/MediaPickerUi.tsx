...
import { SaiFile } from '@lib/mobx-toolbox/saiFileUpload/types';
...
const MediaItemComponent = observer(({
	item,
	imageSize,
	onPress,
	onSelectPress,
			...
			>
				...
				>
					{selectedIndex !== null ? (
						...
						<View style={s.unselectedIndicator} />
					)}
				</SimpleButtonUi>
			</View>
		</TouchableOpacity>
...
export const MediaPickerUi = observer(forwardRef<MediaPickerRef, MediaPickerUiProps>(({
	...
	}, [isVisible, permissionResponse?.granted, onOpen]);
	...
	const handleDonePress = useCallback(async () => {
		...
		const mediaWithFiles: MediaItem[] = await Promise.all(selectedMedia.map(async (media, index) => {
			...
			const uriWithoutFragment = finalUri.split('#')[0].split('?')[0];
			const extension = uriWithoutFragment.split('.').pop() || 'jpg';
								...
								>
									...
									>
										<MainText style={{ color: currentTheme.primary_100 }}>
											{t("ready")} {selectedMedia?.length > 0 && `(${selectedMedia.length})`}
										</MainText>
									</SimpleButtonUi>
								</View>
								...
								>
									{isInitialLoading ? (
										...
										<FlashList
											...
											extraData={selectedMedia.map(m => m.id).join(',')}
