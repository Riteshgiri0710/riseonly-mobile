...
export const RadiusSettings = observer(() => {
	const { currentTheme, setThemeValue, defaultTheme } = themeStore;
	const { t } = useTranslation();

	const radiusValues = [0, 3, 5, 10, 15, 20, 25, 30, 35, 40];
	const currentRadius = pxNative(currentTheme.radius_100);

	const handleChange = (value: number) => setThemeValue('radius_100', `${value}px`);
	const handleReset = () => setThemeValue('radius_100', defaultTheme.radius_100);

	return (
		<ProfileSettingsWrapper
			tKey='settings_radius'
			height={45}
		>
			<Box
				flex={1}
